...
- **FR-003**: System MUST discover all existing chunk files from the blockchain data directory on startup, validate the contiguous prefix, load only the active (latest) chunk into memory, and rebuild indexes by scanning all valid chunk files.
...
- **FR-004**: System MUST detect and report corrupted or unreadable chunk files on startup, identifying the chunk number and file path in log output.
...
- **FR-011**: System MUST load only a contiguous prefix of chunks when gaps are detected in the chunk file sequence.
...
- **SC-002**: Chunk auto-save completes within 2 seconds of a chunk reaching capacity during normal operation.
- **SC-003**: Startup recovery discovers all persisted chunks, loads the active chunk, rebuilds indexes, and the node is fully operational (accepting new blocks and queries) within 30 seconds for chains up to 10,000 blocks.
